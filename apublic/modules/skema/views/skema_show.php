...
<table class="table table-lg table-hover table-bordered table-striped" id="tabel_skema">
...
<?php
	...
	foreach($skema as $dataview){
		$no++;
;?>
<tr>
<tr id="tr_<?php echo $dataview->id_skema;?>"  style="background:<?php echo ($no%2==0)?'#f4f6f9':'';?>">
	<td onclick="selectCb('<?php echo $dataview->id_skema;?>');" >
		<div class="custom-control custom-checkbox">
			<input type="checkbox" class="custom-control-input" name="cb[]" id="cb_<?php echo $dataview->id_skema;?>" value="<?php echo $dataview->id_skema;?>" />
			<label class="custom-control-label" for="cb_<?php echo $dataview->id_skema;?>"> <?php echo $no;?></label>
       </div>
	</td>
	<td><?php
		echo ($dataview->jenis_skema=="PENELITIAN")? '<span class="text-danger font-weight-bold text-small">[ '.$dataview->jenis_skema.' ]</span>':'<span class="text-primary font-weight-bold text-small">[ '.$dataview->jenis_skema.' ]</span>';
		echo '<br/>'.$dataview->nama_skema;?></td>
	<td>
		<button type="button" class="edit btn btn-primary btn-sm" rel="<?php echo $dataview->id_skema;?>"  title="Edit Data"><i class="fa fa-pencil"></i></button>
		<button type="button" class="delete btn btn-danger btn-sm" rel="<?php echo $dataview->id_skema;?>"  title="Delete Data"><i class="fa fa-remove"></i></button>
	</td>
</tr>
<?php };?>
</tbody>
...
function actionAll(act){
	if(item_global.length>0){ ///untuk cek apakah ada record dipilih atau tidak
		...
		$.ajax({
			url       : "<?php echo site_url('skema/skema_actionAll');?>/"+act,
			type      : "POST",
			dataType  : "html",
...
$(document).ready(function(){
	$(".edit").click(function(){
		...
		$.ajax({
				url       : "<?php echo site_url('skema/skema_upd');?>/"+id,
				dataType  : "html",
				beforeSend: function(){
							  $("#ajax_loader").fadeIn(100);
				},
				success   : function(data){
							...
							$("#modalTitle").html("Edit Data Skema");
	...
	$(".delete").click(function(){
		...
		if(confirm("Apakah anda yakin akan menghapus data ?")==true){
			$.ajax({
				url       : "<?php echo site_url('skema/skema_dlt');?>/"+id,
				dataType  : "html",
				beforeSend: function(){
							  $("#ajax_loader").fadeIn(100);
				},
