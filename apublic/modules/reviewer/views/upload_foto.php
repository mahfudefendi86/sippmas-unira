...
<form class="uk-form uk-form-stacked" name="imageUploadForm" id="imageUploadForm" action="<?php echo site_url("reviewer/save_foto");?>" method="post" enctype="multipart/form-data" >
<input type="hidden" class="uk-width-medium-1-1" value="<?php echo (!$is_edit) ? '' : $reviewer->id_user;?>" name="id_user" id="id_user" placeholder="id_user"   />
<table class="table table-striped table-sm">
	<tr>
		<th width="20%">Nama Reviewer</th>
		<td>: <span class="uk-text-danger uk-text-large"><?php echo $reviewer->nama;?></span></td>
	</tr>
	<tr>
		<th width="20%">NIDN/NIP</th>
		<td>: <?php echo $reviewer->nidn;?></td>
	</tr>
	<tr>
		<th width="20%">Program Studi</th>
		<td>: <?php echo $reviewer->nama_prodi?></td>
	</tr>
	<tr>
		<th width="20%">Tempat, Tanggal Lahir</th>
		<td>: <?php echo $reviewer->tempat_lahir.', '.tgl_indo($reviewer->tgl_lahir);?></td>
	</tr>
	<tr>
		<th width="20%">Alamat</th>
		<td>: <?php echo $reviewer->alamat.' - '.$reviewer->nama_desa.' - '.$reviewer->nama_kecamatan.' - '.$reviewer->nama_kota?></td>
	</tr>
</table>
<div id="notif"></div>
<div class="card mb-3">
	<div class="card-body">
		...
		<?php if($reviewer->foto=="" || $reviewer->foto==NULL){
		...
		?>
			...
			$j=$reviewer->foto;
			$sumber=base_url().$j;?>
			<a href="<?php echo $sumber;?>" data-fancybox title="<?php echo $reviewer->nama;?>">
			<img class="img-thumbnail" src="<?php echo base_url().$reviewer->foto_thumb;?>" align="absmiddle" alt="Foto" title="Foto" width="200px" /></a>
			<br/>
			<button type="button" name="hapus" class="btn btn-danger btn-sm mt-2"  onclick="hapus_foto('<?php echo $reviewer->id_user;?>');"><i class="fa fa-remove"></i> Hapus</button>
		<?php }; ?>
		</div>
	</div>
</div>
        ...
        $.ajax({
            ...
            success:function(data){
					var html="";
      				obj = JSON.parse(data);
      				if(obj.status=='OK'){
						html=html+'<a href="<?php echo base_url();?>'+obj.img+'" data-fancybox title="<?php echo addslashes($reviewer->nama);?>">';
      				 	html=html+'<img class="img-thumbnail" src="<?php echo base_url();?>/'+obj.img+'" align="absmiddle" alt="Foto" title="Foto" width="200px"  /></a><br/>';
						html=html+'<button type="button" name="hapus" class="btn btn-sm btn-danger mt-2" onclick="hapus_foto(\'<?php echo $reviewer->id_user;?>\');"><i class="fa fa-remove"></i> Hapus</button>';
...
function hapus_foto(id){
	var r=confirm("Apakah anda yakin akan menghapus data ini ?");
		if(r==true){
			$.ajax({
				url       : "<?php echo site_url('reviewer/foto_upload_remove');?>",
				type	  : "POST",
				data      : "id="+id,
				dataType  : "html",
				beforeSend: function(){
