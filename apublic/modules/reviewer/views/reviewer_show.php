	...
	<td>
		<div class="dropdown">
		  ...
		  <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
  		    ...
  		    <a class="dropdown-item" href="<?php echo site_url('reviewer/foto/'.$dataview->id_user);?>"><i class="fa fa-image"></i> Upload Foto</a>
...
function actionAll(act){
	if(item_global.length>0){ ///untuk cek apakah ada record dipilih atau tidak
		...
		$.ajax({
			url       : "<?php echo site_url('reviewer/reviewer_actionAll');?>/"+act,
			type      : "POST",
			dataType  : "html",
...
$(document).ready(function(){
	$(".setup_login").click(function(){
		...
		$.ajax({
				url       : "<?php echo site_url('userlogin/userlogin_add/r');?>/"+id_user,
				dataType  : "html",
				beforeSend: function(){
							  $("#ajax_loader").fadeIn(100);
				},
				success   : function(data){
							...
							$("#modalTitle").html("Setup Userlogin Reviewer");
	...
	$(".edit").click(function(){
		...
		$.ajax({
				url       : "<?php echo site_url('reviewer/reviewer_upd');?>/"+id,
				dataType  : "html",
				beforeSend: function(){
							  $("#ajax_loader").fadeIn(100);
				},
				success   : function(data){
							$("#ajax_loader").fadeOut(100);
							$("#dataview_modal").html(data);
							$("#modalTitle").html("Edit Data Reviewer");
							$("#modalView").modal("show")
				}
			}); //end Of Ajax
	...
	$(".delete").click(function(){
		...
		if(confirm("Apakah anda yakin akan menghapus data ?")==true){
			$.ajax({
				url       : "<?php echo site_url('reviewer/reviewer_dlt');?>/"+id,
				dataType  : "html",
				beforeSend: function(){
							  $("#ajax_loader").fadeIn(100);
				},
