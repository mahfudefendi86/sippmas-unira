...
if(isset($title)){ echo '<h3 class="text-primary">'.$title.'</h3>';};
?>
<div id="panel_info_update"></div>
<div class="card p-3 mb-3">
<form class="form-horizontal" role="form" name="formpeneliti" id="peneliti" action="<?php echo (!$is_edit) ? site_url("reviewer/reviewer_add") : site_url("reviewer/reviewer_upd").'/'.$reviewer->id_user;?>" method="post" >
<input type="hidden" class="form-control" value="<?php echo (!$is_edit) ? '' : $reviewer->id_user;?>" name="pnl_id_user" id="pnl_id_user" placeholder="id_user"   />
	<div class="form-group row">
		<label class="col-sm-12 col-md-3" for="pnl_nama_lengkap">Nama Lengkap</label>
		<div class="col-sm-12 col-md-9">
			<input type="text" class="form-control" value="<?php echo (!$is_edit) ? '' : $reviewer->nama;?>" name="pnl_nama_lengkap" id="pnl_nama_lengkap" placeholder="Nama Lengkap"   />
		</div>
	</div>
	...
	<div class="form-group row">
		...
		<div class="col-sm-12 col-md-9">
			<input type="text" class="form-control" value="<?php echo (!$is_edit) ? '' : $reviewer->nidn;?>" name="pnl_nidn" id="pnl_nidn" placeholder="NIDN"   />
		</div>
	</div>
	<div class="form-group row">
		<label class="col-sm-12 col-md-3" for="pnl_nidn">NIY <span class="h4 text-danger text-bold">*</span></label>
		<div class="col-sm-12 col-md-9">
			<input type="text" class="form-control" value="<?php echo (!$is_edit) ? '' : $reviewer->niy;?>" name="pnl_niy" id="pnl_niy" placeholder="Nomer Induk Yayasan (NIY)"   />
		</div>
	</div>
	...
	<div class="form-group row">
		...
		<div class="col-sm-12 col-md-9">
			<textarea name="pnl_alamat" id="pnl_alamat"   class="form-control" placeholder="Alamat" ><?php echo (!$is_edit) ? '' : $reviewer->alamat;?></textarea>
		</div>
	</div>
            	...
            	<?php
					if(isset($kota_kab)){
						foreach($kota_kab as $data_kota_kab){
							if($data_kota_kab->id==((!$is_edit) ? '' : $reviewer->kota_kab)){
            	...
            	<?php
					if(isset($kecamatan)){
						foreach($kecamatan as $data_kecamatan){
							if($data_kecamatan->id==((!$is_edit) ? '' : $reviewer->kecamatan)){
            	...
            	<?php
					if(isset($desa)){
						foreach($desa as $data_desa){
							if($data_desa->id==((!$is_edit) ? '' : $reviewer->desa)){
	...
	<div class="form-group row">
		...
		<div class="col-sm-12 col-md-9">
			<input type="email" class="form-control" value="<?php echo (!$is_edit) ? '' : $reviewer->email;?>" name="pnl_email" id="pnl_email" placeholder="Email"    />
		</div>
	</div>
	...
	<div class="form-group row">
		...
		<div class="col-sm-12 col-md-9">
			<input type="text" class="form-control" value="<?php echo (!$is_edit) ? '' : $reviewer->no_hp;?>" name="pnl_nomer_hp" id="pnl_nomer_hp" placeholder="Nomer HP"   />
		</div>
	</div>
	...
	<div class="form-group row">
		...
		<div class="col-sm-12 col-md-9">
			<input type="text" class="form-control" value="<?php echo (!$is_edit) ? '' : $reviewer->tempat_lahir;?>" name="pnl_tempat_lahir" id="pnl_tempat_lahir" placeholder="Tempat Lahir"   />
		</div>
	</div>

	<div class="form-group row">
		<label class="col-sm-12 col-md-3" for="pnl_tanggal_lahir">Tanggal Lahir</label>
		<div class="col-sm-12 col-md-9">
			<div class="input-group date">
				<div class="input-group-addon input-group-prepend">
				    	<span class="input-group-text"><i class="fa fa-fw fa-calendar"></i></span>
				  	</div>
			  <input type="text" class="form-control"  value="<?php echo (!$is_edit) ? '' : $reviewer->tgl_lahir;?>" name="pnl_tanggal_lahir" id="pnl_tanggal_lahir" placeholder="Tanggal Lahir"  >
			</div>
		</div>
	</div>
            	...
            	<?php
					if(isset($fakultas)){
						foreach($fakultas as $data_id_fakultas){
							if($data_id_fakultas->id_fakultas==((!$is_edit) ? '' : $reviewer->fakultas)){
            	...
            	<?php
					if(isset($prodi)){
						foreach($prodi as $data_prodi){
							if($data_prodi->id_prodi==((!$is_edit) ? '' : $reviewer->prodi)){
            	...
            	<?php
					if(isset($keahlian)){
						foreach($keahlian as $data_keahlian){
							if($data_keahlian->id_bidangkeahlian == ((!$is_edit) ? '' : $reviewer->bidang_keahlian) ){
						...
						//echo '<option value="" onclick="$(this).val(\'\');">[ Pilih Prodi Dahulu]</option>';
					}
				?>
			</select>
		</div>
            	...
            	<?php
					if(isset($keilmuan)){
						foreach($keilmuan as $data_ilmu){
							if($data_ilmu->id_bidangilmu==((!$is_edit) ? '' : $reviewer->bidang_ilmu)){
		...
		$.ajax({
			...
			success   : function(data){
					...
					if(obj.status=='OK'){
						...
						<?php
							if($this->session->userdata("akses")=="ADM" || $this->session->userdata("akses")=="SUA"){
								echo 'reload_data();';
								echo '$("#alert_info").html(obj.msg);';
							}else{
								echo '$("#panel_info_update").html(obj.msg);';
								echo 'alert("Data profil berhasil di-update...");';
							}
						?>
					}else
					if(obj.status=='ERROR'){
