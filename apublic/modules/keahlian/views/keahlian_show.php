...
<table class="table table-sm table-hover table-bordered table-striped" id="tabel_keahlian">
...
<?php
	...
	foreach($keahlian as $dataview){
		$no++;
;?>
<tr>
<tr id="tr_<?php echo $dataview->id_bidangkeahlian;?>"  style="background:<?php echo ($no%2==0)?'#f4f6f9':'';?>">
	<td onclick="selectCb('<?php echo $dataview->id_bidangkeahlian;?>');" >
		<div class="custom-control custom-checkbox">
			<input type="checkbox" class="custom-control-input" name="cb[]" id="cb_<?php echo $dataview->id_bidangkeahlian;?>" value="<?php echo $dataview->id_bidangkeahlian;?>" />
			<label class="custom-control-label" for="cb_<?php echo $dataview->id_bidangkeahlian;?>"> <?php echo $no;?></label>
       </div>
	</td>
	<td><?php echo $dataview->nama_bidang;?></td>
	<td>
			<button type="button" class="edit btn btn-primary btn-sm" rel="<?php echo $dataview->id_bidangkeahlian;?>"  title="Edit Data"><i class="fa fa-pencil"></i></button>
			<button type="button" class="delete btn btn-danger btn-sm" rel="<?php echo $dataview->id_bidangkeahlian;?>"  title="Delete Data"><i class="fa fa-remove"></i></button>
...
function actionAll(act){
	if(item_global.length>0){ ///untuk cek apakah ada record dipilih atau tidak
		...
		$.ajax({
			url       : "<?php echo site_url('keahlian/keahlian_actionAll');?>/"+act,
			type      : "POST",
			dataType  : "html",
...
$(document).ready(function(){
	$(".edit").click(function(){
		...
		$.ajax({
				url       : "<?php echo site_url('keahlian/keahlian_upd');?>/"+id,
				dataType  : "html",
				beforeSend: function(){
							  $("#ajax_loader").fadeIn(100);
				},
				success   : function(data){
							...
							$("#modalTitle").html("Edit Data Keahlian");
	...
	$(".delete").click(function(){
		...
		if(confirm("Apakah anda yakin akan menghapus data ?")==true){
			$.ajax({
				url       : "<?php echo site_url('keahlian/keahlian_dlt');?>/"+id,
				dataType  : "html",
				beforeSend: function(){
							  $("#ajax_loader").fadeIn(100);
				},
