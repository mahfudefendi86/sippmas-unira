...
<form class="form-horizontal" role="form" name="formberkas" id="formberkas" action="<?php echo site_url("capaian_seminar/dokumen_save");?>" method="post" enctype="multipart/form-data" >
<input type="hidden" class="form-control" value="<?php echo (!$is_edit) ? '' : $capaian_seminar->id_seminar;?>" name="id_seminar" id="id_seminar" placeholder="id_seminar"   />
<input type="hidden" class="form-control" value="<?php echo (!$is_edit) ? '' : $capaian_seminar->id_penelitian;?>" name="id_penelitian" id="id_penelitian" placeholder="id_penelitian"   />
	<div class="form-group row">
		<label class="col-sm-12 col-md-3" for="userfile">Pilih Berkas <span class="text-danger">*</span></label>
		<div class="col-sm-12 col-md-9">
			<input type="file" class="form-control" value="<?php echo (!$is_edit) ? '' : $capaian_seminar->file_url;?>" name="userfile" id="userfile" placeholder="userfile"   />
		</div>
	</div>
			...
			wrapper: "span",
			 ...
			 messages:{
		  	...
		  	submitHandler: function(form) {
		        ...
		        $.ajax({
		            type:'POST',
		            url: "<?php echo site_url("capaian_seminar/dokumen_save") ;?>",
		            data:formData,
		            cache:false,
		            contentType: false,
		            processData: false,
