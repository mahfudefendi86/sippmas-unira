...
<table class="table table-sm table-hover table-bordered table-striped" id="tabel_fakultas">
...
<?php
	...
	foreach($fakultas as $dataview){
		$no++;
;?>
<tr>
<tr id="tr_<?php echo $dataview->id_fakultas;?>"  style="background:<?php echo ($no%2==0)?'#f4f6f9':'';?>">
	<td onclick="selectCb('<?php echo $dataview->id_fakultas;?>');" >
		<div class="custom-control custom-checkbox">
			<input type="checkbox" class="custom-control-input" name="cb[]" id="cb_<?php echo $dataview->id_fakultas;?>" value="<?php echo $dataview->id_fakultas;?>" />
			<label class="custom-control-label" for="cb_<?php echo $dataview->id_fakultas;?>"> <?php echo $no;?></label>
       </div>
	</td>
	<td><?php echo $dataview->nama_fakultas;?></td>
	<td>
			<button type="button" class="edit btn btn-primary btn-sm" rel="<?php echo $dataview->id_fakultas;?>"  title="Edit Data"><i class="fa fa-pencil"></i></button>
			<button type="button" class="delete btn btn-danger btn-sm" rel="<?php echo $dataview->id_fakultas;?>"  title="Delete Data"><i class="fa fa-remove"></i></button>
...
function actionAll(act){
	if(item_global.length>0){ ///untuk cek apakah ada record dipilih atau tidak
		...
		$.ajax({
			url       : "<?php echo site_url('fakultas/fakultas_actionAll');?>/"+act,
			type      : "POST",
			dataType  : "html",
...
$(document).ready(function(){
	$(".edit").click(function(){
		...
		$.ajax({
				url       : "<?php echo site_url('fakultas/fakultas_upd');?>/"+id,
				dataType  : "html",
				beforeSend: function(){
							  $("#ajax_loader").fadeIn(100);
				},
				success   : function(data){
							...
							$("#modalTitle").html("Edit Data Fakultas");
	...
	$(".delete").click(function(){
		...
		if(confirm("Apakah anda yakin akan menghapus data ?")==true){
			$.ajax({
				url       : "<?php echo site_url('fakultas/fakultas_dlt');?>/"+id,
				dataType  : "html",
				beforeSend: function(){
							  $("#ajax_loader").fadeIn(100);
				},
