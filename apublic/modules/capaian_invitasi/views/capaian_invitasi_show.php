...
<table class="table table-sm table-hover table-bordered table-striped" id="tabel_capaian_invitasi">
...
<?php
	...
	foreach($capaian_invitasi as $dataview){
		$no++;
;?>
<tr>
<tr id="tr_<?php echo $dataview->id_inv_speaker;?>"  style="background:<?php echo ($no%2==0)?'#f4f6f9':'';?>">
	<td onclick="selectCb('<?php echo $dataview->id_inv_speaker;?>');" >
		<div class="custom-control custom-checkbox">
			<input type="checkbox" class="custom-control-input" name="cb[]" id="cb_<?php echo $dataview->id_inv_speaker;?>" value="<?php echo $dataview->id_inv_speaker;?>" />
			<label class="custom-control-label" for="cb_<?php echo $dataview->id_inv_speaker;?>"> <?php echo $no;?></label>
       </div>
	</td>
	<td><?php echo $dataview->id_inv_speaker;?></td>
	<td><?php echo $dataview->id_penelitian;?></td>
	<td><?php echo $dataview->skala_kegiatan;?></td>
	<td><?php echo $dataview->judul_makalah;?></td>
	<td><?php echo $dataview->penulis;?></td>
	<td><?php echo $dataview->penyelenggara;?></td>
	<td><?php echo $dataview->tempat;?></td>
	<td><?php echo $dataview->tanggal;?></td>
	<td><?php echo $dataview->status_makalah;?></td>
	
	<td>
		<button type="button" class="edit btn btn-primary btn-sm" rel="<?php echo $dataview->id_inv_speaker;?>"  title="Edit Data"><i class="fa fa-pencil"></i></button>
		<button type="button" class="delete btn btn-danger btn-sm" rel="<?php echo $dataview->id_inv_speaker;?>"  title="Delete Data"><i class="fa fa-remove"></i></button>
	</td>
</tr>
<?php };?>
</tbody>
...
function actionAll(act){
	if(item_global.length>0){ ///untuk cek apakah ada record dipilih atau tidak
		...
		$.ajax({
			url       : "<?php echo site_url('capaian_invitasi/capaian_invitasi_actionAll');?>/"+act,
			type      : "POST",
			dataType  : "html",
...
$(document).ready(function(){
	...
	$(".edit").click(function(){
		...
		$.ajax({
				url       : "<?php echo site_url('capaian_invitasi/capaian_invitasi_upd');?>/"+id,
				dataType  : "html",
				beforeSend: function(){
							  $("#ajax_loader").fadeIn(100);
				},
				success   : function(data){
							...
							$("#modalTitle").html("Edit Data Capaian_invitasi");
	...
	$(".delete").click(function(){
		...
		if(confirm("Apakah anda yakin akan menghapus data ?")==true){
			$.ajax({
				url       : "<?php echo site_url('capaian_invitasi/capaian_invitasi_dlt');?>/"+id,
				dataType  : "html",
				beforeSend: function(){
							  $("#ajax_loader").fadeIn(100);
				},
