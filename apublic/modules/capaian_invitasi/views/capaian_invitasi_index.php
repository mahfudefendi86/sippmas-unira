...
function reload_data(url){
	...
	if(url == null || url == undefined){
		seturl="<?php echo site_url('capaian_invitasi/capaian_invitasi_show');?>/0";
	}else{
		seturl=url;
	};
	$.ajax({
...
$("#add").click(function(){
	$.ajax({
			url       : "<?php echo site_url('capaian_invitasi/capaian_invitasi_add');?>",
			dataType  : "html",
			beforeSend: function(){
						  $("#ajax_loader").fadeIn(100);
			},
			success   : function(data){
						$("#ajax_loader").fadeOut(100);
						$("#modalTitle").html("<h4>Tambah Data Capaian_invitasi</h4>");
...
$("#search").click(function(){
	$.ajax({
			url       : "<?php echo site_url('capaian_invitasi/capaian_invitasi_search');?>",
			dataType  : "html",
			beforeSend: function(){
						  $("#ajax_loader").fadeIn(100);
			},
			success   : function(data){
						$("#dataview_modal").html(data);
						$("#modalTitle").html("<h4>Cari Data Capaian_invitasi</h4>");
						$("#modalView").modal("show");
						$("#ajax_loader").fadeOut(100);
			}
		});
