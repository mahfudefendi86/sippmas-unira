...
<table class="table table-sm table-hover table-bordered table-striped" id="tabel_capaian_visiting">
...
<?php
	...
	foreach($capaian_visiting as $dataview){
		$no++;
;?>
<tr>
<tr id="tr_<?php echo $dataview->id_visit_scientist;?>"  style="background:<?php echo ($no%2==0)?'#f4f6f9':'';?>">
	<td onclick="selectCb('<?php echo $dataview->id_visit_scientist;?>');" >
		<div class="custom-control custom-checkbox">
			<input type="checkbox" class="custom-control-input" name="cb[]" id="cb_<?php echo $dataview->id_visit_scientist;?>" value="<?php echo $dataview->id_visit_scientist;?>" />
			<label class="custom-control-label" for="cb_<?php echo $dataview->id_visit_scientist;?>"> <?php echo $no;?></label>
       </div>
	</td>
	<td><?php echo $dataview->id_visit_scientist;?></td>
	<td><?php echo $dataview->id_penelitian;?></td>
	<td><?php echo $dataview->skala_kegiatan;?></td>
	<td><?php echo $dataview->nama_perguruan_tinggi;?></td>
	<td><?php echo $dataview->lama_kegiatan;?></td>
	<td><?php echo $dataview->kegiatan_yg_dilakukan;?></td>
	
	<td>
		<button type="button" class="edit btn btn-primary btn-sm" rel="<?php echo $dataview->id_visit_scientist;?>"  title="Edit Data"><i class="fa fa-pencil"></i></button>
		<button type="button" class="delete btn btn-danger btn-sm" rel="<?php echo $dataview->id_visit_scientist;?>"  title="Delete Data"><i class="fa fa-remove"></i></button>
	</td>
</tr>
<?php };?>
</tbody>
...
function actionAll(act){
	if(item_global.length>0){ ///untuk cek apakah ada record dipilih atau tidak
		...
		$.ajax({
			url       : "<?php echo site_url('capaian_visiting/capaian_visiting_actionAll');?>/"+act,
			type      : "POST",
			dataType  : "html",
...
$(document).ready(function(){
	...
	$(".edit").click(function(){
		...
		$.ajax({
				url       : "<?php echo site_url('capaian_visiting/capaian_visiting_upd');?>/"+id,
				dataType  : "html",
				beforeSend: function(){
							  $("#ajax_loader").fadeIn(100);
				},
				success   : function(data){
							...
							$("#modalTitle").html("Edit Data Capaian_visiting");
	...
	$(".delete").click(function(){
		...
		if(confirm("Apakah anda yakin akan menghapus data ?")==true){
			$.ajax({
				url       : "<?php echo site_url('capaian_visiting/capaian_visiting_dlt');?>/"+id,
				dataType  : "html",
				beforeSend: function(){
							  $("#ajax_loader").fadeIn(100);
				},
