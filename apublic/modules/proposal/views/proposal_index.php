...
<div class="card mb-3">
    <div class="card-body">
        <table class="table table-sm table-hover table-striped">
            ...
            <tr>
                <td></td><td>&nbsp;<a data-toggle="tooltip" title="Tampilkan Detail" href="<?php echo site_url('penelitian/detail/'.$penelitian->id_penelitian);?>" class="btn btn-sm btn-success" ><i class="fa fa-cogs"></i> Detail Usulan</a></td>
            </tr>
        </table>
    </div>
</div>
<div class="row">
   ...
   <div class="col-sm-12 col-md-5">
      <div class="float-md-right">
         <button data-toggle="tooltip" type="button" id="add" title="Unggah Berkas" rel="<?php echo $penelitian->id_penelitian;?>" class="btn btn-primary btn-sm ml-3 my-1" ><span class="fa fa-cloud-upload"></span> Unggah Berkas</button>
         <button type="button" id="refresh" title="Refresh" class="btn btn-info btn-sm ml-2 my-1" onclick="window.location.reload();" ><span class="fa fa-refresh"></span> Refresh</button>
         <button type="button" id="kembali" title="Kembali ke Penelitian" onclick="window.history.back();" class="btn btn-warning btn-sm ml-2 my-1" ><span class="fa fa-chevron-left"></span> Kembali</button>
      </div>
   </div>
</div>
<div class="row my-2">
...
<input type="hidden" name="id_penelitian" id="id_penelitian" value="<?php echo (!$is_edit) ? '' : $penelitian->id_penelitian;?>" />
...
function reload_data(url){
	...
	if(url == null || url == undefined){
		seturl="<?php echo site_url('proposal/proposal_show');?>/0";
	}else{
		seturl=url;
	};
	$.ajax({
...
$("#add").click(function(){
	...
	$.ajax({
			url       : "<?php echo site_url('proposal/proposal_add');?>/"+id,
			dataType  : "html",
			beforeSend: function(){
						  $("#ajax_loader").fadeIn(100);
			},
			success   : function(data){
						$("#ajax_loader").fadeOut(100);
						$("#modalTitle").html("<h4>Tambah Berkas</h4>");
...
<?php }else{ ;?>
	<div class="card">
		<div class="card-body">
			<h3 class="text-danger"><i class="fa fa-warning"></i> Maaf, Data Pengajuan Penelitian tidak ditemukan</h3><br/>
