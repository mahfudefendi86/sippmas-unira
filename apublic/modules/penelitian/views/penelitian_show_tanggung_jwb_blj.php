...
<tr id="tr_<?php echo $dataview->id_penelitian;?>"  style="background:<?php echo ($no%2==0)?'#f4f6f9':'';?>">
	...
	<td>
		<a href="<?php echo site_url('cetak_pdf/format_tgjb/'.$dataview->id_penelitian);?>" class="cetak tip btn btn-primary btn-sm" title="Cetak Format Tanggung Jawab Belanja"><i class="fa fa-file-pdf-o"></i></a>
		<button type="button" rel="<?php echo $dataview->id_penelitian;?>" class="upload tip btn btn-success btn-sm" title="Unggah Tanggung Jawab Belanja"><i class="fa fa-upload"></i></button>
		<?php
		if($dataview->file_tgjb > 0){
			echo '<a href="'.site_url('penelitian/unduh_tgjb/'.$dataview->id_penelitian).'" class="tip btn btn-warning btn-sm" title="Unduh Tanggung Jawab Belanja"><i class="fa fa-download"></i></a>';
		}else{
			echo '<button type="button" class="tip btn btn-secondary btn-sm" title="Unduh Tanggung Jawab Belanja" disabled><i class="fa fa-download"></i></button>';
		};
		?>
	</td>
</tr>
...
$('.tip').tooltip({
	...
	$(".upload").click(function(){
		...
		$.ajax({
				url       : "<?php echo site_url('berkas/berkas_add_tgjb');?>",
				dataType  : "html",
				type      : "POST",
				...
				success   : function(data){
							...
							$("#modalTitle").html("<h4>Unggah Berkas Tanggung Jawab Belanja</h4>");
