...
<div class="table-responsive">
    <form name="form_cb" id="form_cb" class="uk-form" style="margin-bottom:60px;">
        <table class="table table-sm table-hover table-bordered table-striped" id="tabel_kelompok_kkn">
            <thead class="thead-light">
                ...
                <tr>
                    <th>No.</th>
                    <th class="sort" data-field="a" onclick="sorted('a');">Tempat KKN</th>
                    <th class="sort" data-field="b" onclick="sorted('b');">Nama Kelompok KKN</th>
                    <th class="sort" data-field="c" onclick="sorted('c');">Nama Ketua Kelompok</th>
                    <th>Action</th>
                </tr>
            </thead>
            <tbody>
...
foreach ($kelompok_kkn as $dataview) {
    $no++;
    ?>
                <tr id="tr_<?php echo $dataview->id_kelompok; ?>"
                    style="background:<?php echo ($no % 2 == 0) ? '#f4f6f9' : ''; ?>">
                    <td onclick="selectCb('<?php echo $dataview->id_kelompok; ?>');">
                        <div class="custom-control custom-checkbox">
                            <input type="checkbox" class="custom-control-input" name="cb[]"
                                id="cb_<?php echo $dataview->id_kelompok; ?>"
                                value="<?php echo $dataview->id_kelompok; ?>" />
                            <label class="custom-control-label" for="cb_<?php echo $dataview->id_kelompok; ?>">
                                <?php echo $no; ?></label>
                        </div>
                    </td>
                    <td><?php echo $dataview->nama_tempat_lookup; ?></td>
                    <td><?php echo $dataview->nama_kelompok; ?></td>
...
$(".cekAll").click(function(event) {
    if (this.checked) {
        ...
        $(":checkbox").each(function() {
            this.checked = true;
            $("#tabel_kelompok_kkn tbody tr").addClass("select_warna");
            addItem(this.value);
        });
    } else {
        $(":checkbox").each(function() {
            this.checked = false;
            $("#tabel_kelompok_kkn tbody tr").removeClass("select_warna");
...
function actionAll(act) {
    if (item_global.length > 0) { ///untuk cek apakah ada record dipilih atau tidak
        ...
        $.ajax({
            url: "<?php echo site_url('kelompok_kkn/kelompok_kkn_actionAll'); ?>/" + act,
            type: "POST",
            dataType: "html",
...
$(document).ready(function() {
    ...
    $(".edit").click(function() {
        ...
        $.ajax({
            url: "<?php echo site_url('kelompok_kkn/kelompok_kkn_upd'); ?>/" + id,
            dataType: "html",
            beforeSend: function() {
                $("#ajax_loader").fadeIn(100);
            },
            success: function(data) {
                ...
                $("#modalTitle").html("Edit Data Kelompok KKN");
    ...
    $(".delete").click(function() {
        ...
        if (confirm("Apakah anda yakin akan menghapus data ?") == true) {
            $.ajax({
                url: "<?php echo site_url('kelompok_kkn/kelompok_kkn_dlt'); ?>/" + id,
                dataType: "html",
                beforeSend: function() {
                    $("#ajax_loader").fadeIn(100);
                },
