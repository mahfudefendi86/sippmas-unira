...
<table class="table table-sm table-hover table-bordered table-striped" id="tabel_plotting_kkn">
...
<tr>
	...
	<th>No.</th>
	<th class="sort" data-field="a" onclick="sorted('a');" >Nama Kelompok</th>
	<th class="sort" data-field="b" onclick="sorted('b');" >Anggota Kelompok</th>
	<th>Action</th>
</tr>
	</thead>
<tbody>
...
foreach ($plotting_kkn as $dataview) {
    $no++;
    ?>
<tr id="tr_<?php echo $dataview->id_plot; ?>"  style="background:<?php echo ($no % 2 == 0) ? '#f4f6f9' : ''; ?>">
	<td onclick="selectCb('<?php echo $dataview->id_plot; ?>');" >
		<div class="custom-control custom-checkbox">
			<input type="checkbox" class="custom-control-input" name="cb[]" id="cb_<?php echo $dataview->id_plot; ?>" value="<?php echo $dataview->id_plot; ?>" />
			<label class="custom-control-label" for="cb_<?php echo $dataview->id_plot; ?>"> <?php echo $no; ?></label>
       </div>
	</td>
	<td><?php echo $dataview->nama_kelompok_lookup; ?></td>
	<td><?php echo $dataview->nama_lookup; ?></td>

	<td>
		<button type="button" class="edit btn btn-primary btn-sm" rel="<?php echo $dataview->id_plot; ?>"  title="Edit Data"><i class="fa fa-pencil"></i></button>
		<button type="button" class="delete btn btn-danger btn-sm" rel="<?php echo $dataview->id_plot; ?>"  title="Delete Data"><i class="fa fa-remove"></i></button>
	</td>
</tr>
<?php }
;?>
...
<div  style="margin-top:20px!important;position:relative;"  id="link_pagination" class="float-md-right"><?php echo $links ?></div>
...
$(".cekAll").click(function(event) {
  if(this.checked) {
      ...
      $(":checkbox").each(function() {
          this.checked = true; $("#tabel_plotting_kkn tbody tr").addClass( "select_warna" );
		  addItem(this.value);
      });
  }
  else {
    $(":checkbox").each(function() {
          this.checked = false; $("#tabel_plotting_kkn tbody tr").removeClass( "select_warna" );
...
function actionAll(act){
	if(item_global.length>0){ ///untuk cek apakah ada record dipilih atau tidak
		...
		$.ajax({
			url       : "<?php echo site_url('plotting_kkn/plotting_kkn_actionAll'); ?>/"+act,
			type      : "POST",
			dataType  : "html",
...
$(document).ready(function(){
	...
	$(".edit").click(function(){
		...
		$.ajax({
				url       : "<?php echo site_url('plotting_kkn/plotting_kkn_upd'); ?>/"+id,
				dataType  : "html",
				beforeSend: function(){
							  $("#ajax_loader").fadeIn(100);
				},
				success   : function(data){
							...
							$("#modalTitle").html("Edit Data Plotting Peserta KKN");
	...
	$(".delete").click(function(){
		...
		if(confirm("Apakah anda yakin akan menghapus data ?")==true){
			$.ajax({
				url       : "<?php echo site_url('plotting_kkn/plotting_kkn_dlt'); ?>/"+id,
				dataType  : "html",
				beforeSend: function(){
							  $("#ajax_loader").fadeIn(100);
				},
