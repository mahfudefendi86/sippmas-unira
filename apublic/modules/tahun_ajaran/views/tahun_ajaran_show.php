...
<div class="table-responsive">
    <form name="form_cb" id="form_cb" class="uk-form" style="margin-bottom:60px;">
        <table class="table table-sm table-hover table-bordered table-striped" id="tabel_tahun_ajaran">
            <thead class="thead-light">
                ...
                <tr>
                    <th>No.</th>
                    <th class="sort" data-field="a" onclick="sorted('a');">Nama KKN</th>
                    <th class="sort" data-field="b" onclick="sorted('b');">Tanggal Mulai</th>
                    <th class="sort" data-field="c" onclick="sorted('c');">Tanggal Selesai</th>
                    <th class="sort" data-field="d" onclick="sorted('d');">Keterangan</th>
                    <th>Action</th>
                </tr>
            </thead>
            <tbody>
...
foreach ($tahun_ajaran as $dataview) {
    $no++;
    ?>
                <tr id="tr_<?php echo $dataview->id_thn_kkn; ?>"
                    style="background:<?php echo ($no % 2 == 0) ? '#f4f6f9' : ''; ?>">
                    <td onclick="selectCb('<?php echo $dataview->id_thn_kkn; ?>');">
                        <div class="custom-control custom-checkbox">
                            <input type="checkbox" class="custom-control-input" name="cb[]"
                                id="cb_<?php echo $dataview->id_thn_kkn; ?>"
                                value="<?php echo $dataview->id_thn_kkn; ?>" />
                            <label class="custom-control-label" for="cb_<?php echo $dataview->id_thn_kkn; ?>">
                                <?php echo $no; ?></label>
                        </div>
                    </td>
                    <td><?php echo $dataview->nama_kkn; ?></td>
                    <td><?php echo tgl_indo($dataview->tgl_mulai); ?></td>
                    <td><?php echo tgl_indo($dataview->tgl_selesai); ?></td>
                    <td><?php echo $dataview->keterangan; ?></td>

                    <td>
                        <button type="button" class="edit btn btn-primary btn-sm"
                            rel="<?php echo $dataview->id_thn_kkn; ?>" title="Edit Data"><i
                                class="fa fa-pencil"></i></button>
                        <button type="button" class="delete btn btn-danger btn-sm"
                            rel="<?php echo $dataview->id_thn_kkn; ?>" title="Delete Data"><i
...
$(".cekAll").click(function(event) {
    if (this.checked) {
        ...
        $(":checkbox").each(function() {
            this.checked = true;
            $("#tabel_tahun_ajaran tbody tr").addClass("select_warna");
            addItem(this.value);
        });
    } else {
        $(":checkbox").each(function() {
            this.checked = false;
            $("#tabel_tahun_ajaran tbody tr").removeClass("select_warna");
...
function actionAll(act) {
    if (item_global.length > 0) { ///untuk cek apakah ada record dipilih atau tidak
        ...
        $.ajax({
            url: "<?php echo site_url('tahun_ajaran/tahun_ajaran_actionAll'); ?>/" + act,
            type: "POST",
            dataType: "html",
...
$(document).ready(function() {
    ...
    $(".edit").click(function() {
        ...
        $.ajax({
            url: "<?php echo site_url('tahun_ajaran/tahun_ajaran_upd'); ?>/" + id,
            dataType: "html",
            beforeSend: function() {
                $("#ajax_loader").fadeIn(100);
            },
            success: function(data) {
                ...
                $("#modalTitle").html("Edit Data Tahun Ajaran");
    ...
    $(".delete").click(function() {
        ...
        if (confirm("Apakah anda yakin akan menghapus data ?") == true) {
            $.ajax({
                url: "<?php echo site_url('tahun_ajaran/tahun_ajaran_dlt'); ?>/" + id,
                dataType: "html",
                beforeSend: function() {
                    $("#ajax_loader").fadeIn(100);
                },
