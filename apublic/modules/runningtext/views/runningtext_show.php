...
<table class="table table-sm table-hover table-bordered table-striped" id="tabel_runningtext">
...
<?php
	...
	foreach($runningtext as $dataview){
		$no++;
		$colortext= ($dataview->color_teks=="" || $dataview->color_teks==NULL || $dataview->color_teks=="transparen") ? '#ffe600': $dataview->color_teks;
;?>
<tr>
<tr id="tr_<?php echo $dataview->id_running;?>"  style="background:<?php echo ($no%2==0)?'#f4f6f9':'';?>">
	<td onclick="selectCb('<?php echo $dataview->id_running;?>');" >
		<div class="custom-control custom-checkbox">
			<input type="checkbox" class="custom-control-input" name="cb[]" id="cb_<?php echo $dataview->id_running;?>" value="<?php echo $dataview->id_running;?>" />
			<label class="custom-control-label" for="cb_<?php echo $dataview->id_running;?>"> <?php echo $no;?></label>
       </div>
	</td>
	<td><?php echo tgl_indo($dataview->start_date);?></td>
	<td><?php echo tgl_indo($dataview->end_date);?></td>
	<td style="background:#2a2a2a;"><?php echo '<span style="color:'.$colortext.'; text-shadow:0px 0px 3px #333333;"><i class="'.$dataview->icon_run.'"></i> '.$dataview->text_to_run.'</span>';?></td>
	<td><?php
	if($dataview->status_run=="Y"){ echo '<a title="Press to stop running text" href="#" class="tip badge badge-success" onclick="update_status(\''.$dataview->id_running.'\',\'N\');"><i class="fa fa-play-circle"></i> Running</a>';}else
	if($dataview->status_run=="N"){ echo '<a title="Press to play running text"  href="#" class="tip badge badge-secondary" onclick="update_status(\''.$dataview->id_running.'\',\'Y\');"><i class="fa fa-play-circle"></i> Running</a>';}else ;?></td>

	<td>
		<button type="button" class="edit btn btn-primary btn-sm" rel="<?php echo $dataview->id_running;?>"  title="Edit Data"><i class="fa fa-pencil"></i></button>
		<button type="button" class="delete btn btn-danger btn-sm" rel="<?php echo $dataview->id_running;?>"  title="Delete Data"><i class="fa fa-remove"></i></button>
	</td>
</tr>
<?php };?>
</tbody>
...
function actionAll(act){
	if(item_global.length>0){ ///untuk cek apakah ada record dipilih atau tidak
		...
		$.ajax({
			url       : "<?php echo site_url('runningtext/runningtext_actionAll');?>/"+act,
			type      : "POST",
			dataType  : "html",
...
$(document).ready(function(){
	...
	$(".edit").click(function(){
		...
		$.ajax({
				url       : "<?php echo site_url('runningtext/runningtext_upd');?>/"+id,
				dataType  : "html",
				beforeSend: function(){
							  $("#ajax_loader").fadeIn(100);
				},
				success   : function(data){
							...
							$("#modalTitle").html("Edit Data Runningtext");
	...
	$(".delete").click(function(){
		...
		if(confirm("Apakah anda yakin akan menghapus data ?")==true){
			$.ajax({
				url       : "<?php echo site_url('runningtext/runningtext_dlt');?>/"+id,
				dataType  : "html",
				beforeSend: function(){
							  $("#ajax_loader").fadeIn(100);
				},
...
function update_status(id,st){
	$.ajax({
		url       : "<?php echo site_url('runningtext/update_status');?>/"+id+"/"+st,
		dataType  : "html",
		beforeSend: function(){
					  $("#ajax_loader").fadeIn(100);
		},
