...
function reload_data(url){
	...
	if(url == null || url == undefined){
		seturl="<?php echo site_url('runningtext/runningtext_show');?>/0";
	}else{
		seturl=url;
	};
	$.ajax({
...
$("#add").click(function(){
	$.ajax({
			url       : "<?php echo site_url('runningtext/runningtext_add');?>",
			dataType  : "html",
			beforeSend: function(){
						  $("#ajax_loader").fadeIn(100);
			},
			success   : function(data){
						$("#ajax_loader").fadeOut(100);
						$("#modalTitle").html("<h4>Tambah Data Running Text</h4>");
