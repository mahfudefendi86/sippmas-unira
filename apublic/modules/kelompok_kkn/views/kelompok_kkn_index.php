<div class="row">
    ...
    <div class="col-sm-12 col-md-5">
        <div class="row justify-content-md-center">
            <a id="add" title="Tambah Daftar Kelompok_kkn"><button class="btn btn-primary btn-sm ml-3 my-1"><span
                        class="fa fa-plus"></span> Tambah Data</button></a>
            <a id="refresh" title="Refresh" onclick="window.location.reload();"><button
                    class="btn btn-info btn-sm ml-2 my-1"><span class="fa fa-refresh"></span> Refresh</button></a>
        </div>
    </div>
</div>
<div class="row my-2">
    <div class="col-md-4 col-sm-12 mb-1">
        <div class="input-group mb-2">
            ...
            <select name="slct_row" id="slct_row" class="form-control" onchange="reload_data_kelompok_kkn();">
...
?>
    ...
    <div class="col-sm-12 col-md-4 mb-1">
        <div class="input-group">
            <input type="text" name="tb_filter" id="tb_filter" class="form-control border-right-0 border"
                placeholder="Kata Kunci Pencarian" onkeyup="switch_icon();">
...
setTimeout("reload_data_kelompok_kkn();", 1);

var linkPage = "<?php echo site_url('kelompok_kkn/kelompok_kkn_show'); ?>/0";
...
function switch_icon(cls) {
    ...
    if (i == "") {
        ...
        $("#icon_search").html(
            '<div style="cursor:pointer;" onclick="switch_icon(\'TRUE\');" class="input-group-text bg-transparent"><i class="fa fa-remove"></i></div>'
            );
    }
    reload_data_kelompok_kkn();
};

function reload_data_kelompok_kkn(url) {
    ...
    $.ajax({
        ...
        type: "POST",
        dataType: "html",
        data: "row=" + row + "&filter=" + filter + "&cari=" + cari + "&sortby=" + sort_field + "&sort=" +
            sort_index,
        beforeSend: function() {
...
$("#add").click(function() {
    $.ajax({
        url: "<?php echo site_url('kelompok_kkn/kelompok_kkn_add'); ?>",
        dataType: "html",
        beforeSend: function() {
            $("#ajax_loader").fadeIn(100);
        },
        success: function(data) {
            $("#ajax_loader").fadeOut(100);
            $("#modalTitle").html("<h4>Tambah Data Kelompok KKN</h4>");
