...
<table class="table table-sm table-hover table-bordered table-striped" id="tabel_userlogin">
...
<?php
	...
	foreach($userlogin as $dataview){
	$no++;
	if($this->session->userdata('akses')!="SUA"){
		if($dataview->identifikasi!="SUA" ){
;?>
<tr id="tr_<?php echo $dataview->id_login;?>"  style="background:<?php echo ($no%2==0)?'#f4f6f9':'';?>">
	<td onclick="selectCb('<?php echo $dataview->id_login;?>');" >
		<div class="custom-control custom-checkbox">
			<input type="checkbox" class="custom-control-input" name="cb[]" id="cb_<?php echo $dataview->id_login;?>" value="<?php echo $dataview->id_login;?>" />
			<label class="custom-control-label" for="cb_<?php echo $dataview->id_login;?>"> <?php echo $no;?></label>
       </div>
	</td>
	<td>
	<?php if(($dataview->is_ada==NULL || $dataview->is_ada=="") && $dataview->identifikasi!="ADM"){
		 echo '<span class="text-danger title="Userlogin belum terkonfigurasi dengan Author/Reviewer" data-toggle="tooltip" font-weight-bold"><span style="text-decoration:line-through;">'.$dataview->username.'</span> <i class="fa fa-exclamation-triangle"></i> </span><br/><span class="text-small text-danger">(Userlogin belum terkonfigurasi dengan Author/Reviewer)</span>';
	}else{
		echo '<span>'.$dataview->username.'</span>';
	}?>
	</td>
	<td><?php echo $dataview->nama;?></td>
	<td><?php echo $dataview->alamat;?></td>
	<td><?php echo $dataview->identifikasi;?></td>
	<td><?php echo $dataview->status;?></td>
	<td>
		<button type="button" class="edit btn btn-primary btn-sm" rel="<?php echo $dataview->id_login;?>"  title="Edit Data"><i class="fa fa-pencil"></i></button>
		<button type="button" class="delete btn btn-danger btn-sm" rel="<?php echo $dataview->id_login;?>"  title="Delete Data"><i class="fa fa-remove"></i></button>
...
function actionAll(act){
	if(item_global.length>0){ ///untuk cek apakah ada record dipilih atau tidak
		...
		$.ajax({
			url       : "<?php echo site_url('userlogin/userlogin_actionAll');?>/"+act,
			type      : "POST",
			dataType  : "html",
...
$(document).ready(function(){
	$(".edit").click(function(){
		...
		$.ajax({
				url       : "<?php echo site_url('userlogin/userlogin_upd');?>/"+id,
				dataType  : "html",
				beforeSend: function(){
							  $("#ajax_loader").fadeIn(100);
				},
				success   : function(data){
							...
							$("#modalTitle").html("Edit Data Userlogin");
	...
	$(".delete").click(function(){
		...
		if(confirm("Apakah anda yakin akan menghapus data ?")==true){
			$.ajax({
				url       : "<?php echo site_url('userlogin/userlogin_dlt');?>/"+id,
				dataType  : "html",
				beforeSend: function(){
							  $("#ajax_loader").fadeIn(100);
				},
